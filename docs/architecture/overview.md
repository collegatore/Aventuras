...
The edge swipes live in `AppShell.svelte` and are deliberately asymmetric. The right one opens the
sidebar whenever it is closed; the left one only while _neither_ panel is open, because a
right-swipe inside the open sidebar already belongs to that sidebar's tab strip and must not also
